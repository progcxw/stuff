...
Page({
  ...
  getGoodsInfo: function() {
    let that = this;
    util.request(api.PostGoodsDetail + '?id=' + that.data.id).then(function(res) {
      if (res.errno === 0) {
        ...
        //计算卖家来平台第几天
        let registerTime = res.data.seller.register_time
        let duration = new Date().getTime() - new Date(registerTime * 1000).getTime();
        let dates = parseInt(Math.floor(duration) / (1000 * 60 * 60 * 24));

        //计算商品发布时间
        let goodsTime = new Date(res.data.info.add_time * 1000);
        let addTime = goodsTime.getFullYear() + "-" + (goodsTime.getMonth()+1) + "-" + goodsTime.getDate();
        ...
        if (that.data.seller.id == wx.getStorageSync('userInfo').id) {
          console.log("当前用户是卖家")
          that.setData({
            isSeller: true
          });
  ...
  talkToSeller: function(event) {
    ...
    user.checkLogin().then(() => {
      util.request(api.ChatOpen, {
        receiver_id: that.data.seller.id,
        goods_id: that.data.id,
      }).then(function(res) {
        if (res.errno === 0) {
          wx.navigateTo({
            url: '/pages/chat/chatForm/chatForm?id=' + res.data.chatId,
          })
        }
      })
    })
